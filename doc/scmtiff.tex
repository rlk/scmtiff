...
\texttt{convert} has the most options of any process, as the parameters specified at the first step are carried through into the final data product. The pages of a converted \scm\ \tiff\ will appear in depth-first order.\pagenote{They're in depth-first order, yes, but only the leaves are represented. Thus, the true ordering is an inelegant space-filling curve.}
...
\bigskip The \texttt{mipmap} process generates the intermediate nodes of an \scm\ tree. In general, the \texttt{convert} process generates tree leaves, the \texttt{combine} process merges trees, and the \texttt{mapmap} process subsamples this output, enabling real-time display of the data at arbitrary resolution. It is important to \texttt{combine} before \texttt{mipmap}ping, as boundary artifacts may remain apparent in the intermediate pages of combined mipmaps.

The \texttt{mipmap} process takes no parameters. In the interest of efficiency, mipmapping occurs \textit{in place}, and subsampled pages are appended to the existing file, and the output file name option is ignored. The \texttt{mipmap} process generates as many subsampled pages as possible, and in the common case, an input with depth $d$ will have $d-1$ levels appended to it.

The pages of a mipmapped \scm\ \tiff\ appear in order of decreasing level, and the order within each level corresponds with the resulting of the conversion or the combining, whichever happend most recently.\pagenote{A newly-converted, mipmapped \scm\ \tiff\ will have a depth-first order within each level. A newly-combined, mipmapped \scm\ \tiff\ will have breadth-first order within each level. Either way, this ordering is essentially useless from any algorithmic standpoint, and the \tiff\ won't meet the \scm\ prefix criterion because it's in reverse order. The expectation is that all mipmapped \scm\ \tiff s will be immediately bordered, which will straighten everything right out.}

\subsection{Border}

\noindent\scmtiff\ \texttt{-p border} [\texttt{-o} \inangles{\textit{output}}] \inangles{\textit{input}}

\bigskip The \texttt{border} process fills the pixels of the outermost rows and columns of each page with the pixels from the adjacent rows or columns of the four neighboring pages. This provides each page with the context needed by the graphics hardware to perform linear magnification filtering across page boundaries, effectively allowing many small images to appear as a single very large image.\pagenote{The replicated borders provide only a single pixel of context. This enables linear magnification filtering, but does \emph{not} enable anisotropic filtering.\footnote{Which sucks.}} If this step is not performed, artifacts will appear in the output. In practice, the \texttt{border} process is expensive, as it must decompress and recompress every page of the \scm. The pages of a bordered \scm\ \tiff\ appear in breadth-first order.
...
The page order of the output will match the page order of the input, and mipmaps will be preserved.\pagenote{Indeed, one should \emph{always} normalize mipmaps instead of mipmapping normals. A mipmapped normal is not guaranteed to have unit length, nor will it necessarily correctly represent the normal averaged over the sample's footprint, as would a normal generated directly from height map samples averaged over their own footprint.} However, the output will \emph{not} be bordered or cataloged.
