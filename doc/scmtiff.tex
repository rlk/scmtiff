...
The calculation of neighbor indices is a bit more complicated. In the trivial case, the approach is identical to the calculation of a parent or child. One has only to increment or decrement the row or column argument to \scmindex{f,\ell,r,c} to determine any adjacency. However, finding neighbors across cube map face boundaries causes complications due to the varying orientations of the roots. The resulting cases are tedious to derive, so they are fully enumerated here for reference and ease of implementation.
