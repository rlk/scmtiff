...
The following example is much more involved, merging multiple data sets captured by the Lunar Reconnaissance Orbiter (LRO) to produce a globally-high-quality height map of the moon. In the next sections, this height map is used to produce a normal map for real-time illumination, and a scripted build process is used to orchestrate the full conversion process automatically. This represents real-world usage of \scmtiff.
...
This height map is based primarily on a 100 meter-per-pixel 16-bit digital terrain model (DTM) derived by stereo reconstruction of imagery captured by the Wide Angle Camera (WAC) of the Lunar Reconnaissance Orbiter Camera (LROC).\pagenote{\url{http://wms.lroc.asu.edu/lroc/global_product/100_mpp_DEM}} This data set, known as the Global Lunar 100 Meter DTM (GLD100), gives beautifully clean terrain data over most of the moon, omitting only the poles where illumination and stereo disparity are insufficient for reconstruction.

In the interest of optimal image quality, GLD100 is provided in several parts, with two different projections. Latitudes below $60^\circ$ are given by eight equirectangular projections, shown in Figure~\ref{fig:glde}. Each of these is $27291\times 18195$ pixels in size, totalling $109164\times 36390$ pixels. Latitudes above $60^\circ$ are given by two stereographic polar projections, shown in Figures~\ref{fig:gldn} and~\ref{fig:glds}. Latitudes beyond $79^\circ$ are not represented.
