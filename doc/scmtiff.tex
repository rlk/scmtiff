...
\item For a given $w$, small $n$ requires large $d$. This increases the total number of pages in flight at any given moment, as well as the number of texture image units consumed by the renderer. This is usually not a problem, but depending upon the hardware, one may encounter a limit.\pagenote{The real limit is the total depth of all simultaneously loaded data sets, which must be less than \verb|GL_MAX_TEXTURE_IMAGE_UNITS|. On a 2012-era Mac, this limit is 16. On a 2012-era NVIDIA-equiped Linux or Windows PC, this limit is 56 or 64.}
