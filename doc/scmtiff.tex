...
The \scmview\ utility takes a list of \scm\ \tiff\ files on the command line, and has no optional arguments.

\bigskip\noindent\scmview\ \inangles{\textit{input}} [\ldots]
...
The polar GLD inputs of Figures~\ref{fig:gldn} and~\ref{fig:glds} have a different projection. They overlap with the equirectangular inputs slightly, and the coverages sum to values larger than $1.0$. Careful scrutiny with \scmview, as in Figure~\ref{fig:scmview}, can reveal issues like this, but usually they only became apparent through trial and error. To resolve the issue, the polar inputs will be combined with the already-merged equatorial inputs using the \texttt{max} operator.
