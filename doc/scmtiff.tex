...
The \api\ provides three primary classes: a \emph{cache} class that manages \scm\ data input, a \emph{model} class that renders the spherical geometry to which the cached imagery is applied, and a \emph{label} class that optionally annotates the sphere and its imagery. An application that renders \scm\ data will require at least one cache object and one model object.
...
\begin{description}
\item[scm\_cache::scm\_cache(int size)]\hfill\\
The cache constructor takes an integer giving the maximum cache size in pages.

\item[int scm\_cache::add\_file(const std::string\& scm)]\hfill\\
Add the named \scm\ \tiff\ file to the cache. The return value will be an integer descriptor for that file.

\item[void scm\_cache::update(int time)]

\item[void scm\_cache::draw()]
